...
{
	PushStack((cpu->prgCounter >> 8), cpu);
	PushStack((cpu->prgCounter & 0xFF), cpu);
...
u8 brk(u8 value, Cpu *cpu)
{
	...
	PushStack(HighByte, cpu);
	u8 LowByte = (u8)cpu->prgCounter;
	PushStack(LowByte, cpu);
...
{
	ReadCpu8(cpu->prgCounter, cpu, 2); // Cycle 2
	cpu->flags = PopStack(cpu); // Cycle 4
	cpu->prgCounter = (cpu->prgCounter & 0xFF00) | PopStack(cpu);
	PollInterrupts(cpu, 5); // Cycle 5
	cpu->prgCounter = (PopStack(cpu) << 8) | (cpu->prgCounter & 0x00FF);
	return(0);
}
...
u8 jsr(u8 value, Cpu *cpu)
{
	...
	PushStack((cpu->prgCounter >> 8), cpu); // TODO: Combine decrement and PushStack
	PushStack(cpu->prgCounter & 0xFF, cpu);

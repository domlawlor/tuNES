...
static void GuiTooltip(Rectangle controlRec)
{
    if (!guiLocked && guiTooltip && (guiTooltipPtr != NULL))
    {
		...
		Rectangle panelRect = {controlRec.x, controlRec.y + controlRec.height + 4, textSize.x + 16, (float)GuiGetStyle(DEFAULT, TEXT_SIZE) + 8.0f};
        ...
        GuiSetStyle(LABEL, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
		Rectangle labelRect = {controlRec.x, controlRec.y + controlRec.height + 4, textSize.x + 16, (float)GuiGetStyle(DEFAULT, TEXT_SIZE) + 8.0f};
        GuiLabel(labelRect, guiTooltipPtr);
